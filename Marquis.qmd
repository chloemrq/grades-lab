...
### Question 26
```{r}
courses_trimester <- courses |> select(`course name`, trimester)

# Create a vector of courses per trimester
trim1_courses <- intersect(courses$`course name`[courses$trimester == 1], colnames(grades_wide))
trim2_courses <- intersect(courses$`course name`[courses$trimester == 2], colnames(grades_wide))
trim3_courses <- intersect(courses$`course name`[courses$trimester == 3], colnames(grades_wide))
...
pass_df <- grades_wide |>
  ...
  mutate(
    ...
    pass = min(c_across(-c(id, group)), na.rm = TRUE) >= 5 &          
           mean(c_across(any_of(trim1_courses)), na.rm = TRUE) >= 10 &    
           mean(c_across(any_of(trim2_courses)), na.rm = TRUE) >= 10 &    
           mean(c_across(any_of(trim3_courses)), na.rm = TRUE) >= 10        
  ) |>
  ungroup() |>
  select(id, group, final_grade, pass) |>
  arrange(desc(final_grade))

pass_df |> slice_head(n = 25) |> knitr::kable()
